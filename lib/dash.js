...
// map :: (a -> b -> n) -> [a] -> [b]
function mapN(f, c, n = Infinity) {
  if (isArray(c)) {
    const length = Math.min(c.length, n);
    const nC = Array(length);

    for (let i = 0; i < length; ++i) nC[i] = f(c[i]);

    return nC;
  }

  const keys = Object.keys(c);
  const length = Math.min(keys.length, n);
  const nC = Array(length);

  for (let j = 0; j < length; ++j) nC[j] = f(c[keys[j]]);
...
function reduce(f, acc, c) {
  ...
  if (isArray(c)) {
    ...
    for (let i = 0; i < length; ++i) nC = f(nC, c[i]);
  ...
  for (let j = 0; j < length; ++j) nC = f(nC, c[keys[j]]);
